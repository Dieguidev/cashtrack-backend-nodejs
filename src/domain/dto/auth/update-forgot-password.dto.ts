export class UpdateForgotPsswordDto {
  private constructor(public password: string, public token: string) {}

  static create(object: {
    [key: string]: any;
  }): [{ [key: string]: string }?, UpdateForgotPsswordDto?] {
    ...
    if (typeof password !== 'string')
      errors.password = 'Invalid password - must be a string';
    ...
    if (typeof token !== 'string')
      errors.token = 'Invalid token - must be a string';

    if (Object.keys(errors).length > 0) {
      return [errors];
    }

    return [undefined, new UpdateForgotPsswordDto(password, token)];
  }
}
